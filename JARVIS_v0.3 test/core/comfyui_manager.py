...
# Bekannte Node-Typen -> Parameter-Mapping
NODE_PARAM_MAP = {
    "CLIPTextEncode": {
...
class ComfyUIWorkflowManager:
    """Lädt ComfyUI Workflows und extrahiert editierbare Parameter."""
    ...
    def _scan_workflows(self):
        """Scannt workflows/ Ordner nach JSON-Dateien."""
    ...
    def _extract_params(self, workflow_data: dict) -> List[Dict]:
        """Extrahiert editierbare Parameter aus Workflow-JSON."""
        params = []
        seen = set()
            ...
            if class_type in NODE_PARAM_MAP:
                for key, meta in NODE_PARAM_MAP[class_type].items():
                    if key in inputs:
                        ...
                        # Vermeide Duplikate (z.B. 2x CLIPTextEncode)
                        if param_id in seen:
